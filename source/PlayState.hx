...
class PlayState extends MusicBeatState
{
	...
	//public static var noteBools:Array<Bool> = [false, false, false, false]; //yes this fix the fast fart
	...
	public var fartStrums:FlxTypedGroup<StrumNote>;
	...
	var dialogue:Array<String> = ['blah blah blah', 'coolswag', 'rip'];
	...
	function callLua(func_name : String, args : Array<Dynamic>, ?type : String) : Dynamic
	{
		...
		if (getLuaErrorMessage(lua) != null)
			if (curSong.toLowerCase() == 'restrained')				
				trace('what');
			else
	...
	override public function create()
	{
		...
		theFunne = theFunne && SONG.song.toLowerCase() != 'skill-issue';
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'unexpectancies':
				dialogue = CoolUtil.coolTextFile(Paths.txt('unexpectancies/dialoguelol'));
			case 'troubled':
				dialogue = CoolUtil.coolTextFile(Paths.txt('troubled/dialoguelol'));
			case 'restrained':
				dialogue = CoolUtil.coolTextFile(Paths.txt('restrained/dialoguelol'));
			case 'jam':
				dialogue = CoolUtil.coolTextFile(Paths.txt('jam/dialogue'));
			case 'chippin':
				dialogue = CoolUtil.coolTextFile(Paths.txt('chippin/dialogue'));
			case 'dejammed':
				dialogue = CoolUtil.coolTextFile(Paths.txt('dejammed/dialogue'));
		}
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'unexpectancies' | 'this-shit-gass':
			{
				curStage = 'unexpectancies';

				defaultCamZoom = 0.90;

				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('shred/new background house'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.2, 0.2);
				bg.active = false;
				bg.setGraphicSize(Std.int(bg.width * 0.8));
			...
			/*case 'jam' | 'chippin': //THE FUNNY THING NIGHT IDK
			{
				curStage = 'UHHHHHHHHH STAGEBACK IDK THE FUCKING NAME OK';

				defaultCamZoom = 0.90;

				var bg:FlxSprite = new FlxSprite(-600, -300).loadGraphic(Paths.image('chips/stagebacknight'));
				bg.antialiasing = true;
				...
				add(bg);
				var bgLMAO:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('shred/stagefrontnight'));
				bgLMAO.antialiasing = true;
				bgLMAO.scrollFactor.set(0.9, 0.9);
				bgLMAO.active = false;
				bgLMAO.setGraphicSize(Std.int(bgLMAO.width * 0.8));
				bgLMAO.updateHitbox();
				bgLMAO.scale.x = 1.1;
				bgLMAO.scale.y = 1.1;
			...
			case 'jam' | 'chippin':
			{
				defaultCamZoom = 0.9;
				curStage = 'chips';
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('chips/stageback','shared'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.9, 0.9);
				bg.active = false;
				add(bg);

				var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('chips/stagefront','shared'));
				...
				var stageCurtains:FlxSprite = new FlxSprite(-500, -300).loadGraphic(Paths.image('chips/stagecurtains','shared'));
			...
			case 'dejammed':
			{
				defaultCamZoom = 0.9;
				curStage = 'CHXPZ';
				var bg:FlxSprite = new FlxSprite(-600, -300).loadGraphic(Paths.image('chips/demon','shared'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.9, 0.9);
				bg.active = false;
				add(bg);

				var bgWHY:FlxSprite = new FlxSprite(-600, -300).loadGraphic(Paths.image('chips/demon2','shared'));
				bgWHY.antialiasing = true;
				bgWHY.scrollFactor.set(0.9, 0.9);
				bgWHY.active = false;
				add(bgWHY);
			...
			case 'troubled':
			{
				...
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('shred/troubled background'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.2, 0.2);
				bg.active = false;
				bg.setGraphicSize(Std.int(bg.width * 0.8));
			...
			case 'restrained' | 'house':
			{
				curStage = 'restrained';

				defaultCamZoom = 0.90;

				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('shred/refrained background'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.2, 0.2);
				bg.active = false;
				bg.setGraphicSize(Std.int(bg.width * 0.8));
			...
			case 'bathroom':
			{
				...
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('shred/bathroom  background'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.2, 0.2);
				bg.active = false;
				bg.setGraphicSize(Std.int(bg.width * 0.8));
		...
		switch (SONG.player2)
		{
			...
			case 'spirit':
				...
				dad.y += 100;
				camPos.set(dad.getGraphicMidpoint().x + 300, dad.getGraphicMidpoint().y);
			case 'shredboi' | 'hell-shredboi' | 'shredboi-hd':
				dad.y += 240;
				dad.x += 110;
			case 'chips': //why i put? ok so, whoever made the fucking sprite, made it big, why? I DONT FUCKING KNOW.
				dad.scale.x -= 0.5;
				dad.scale.y -= 0.5;
		}
		...
		switch (curStage)
		{
			...
			case 'schoolEvil':
				...
				gf.x += 180;
				gf.y += 300;
			case 'restrained' | 'bathroom':
				var DIEDIE = new FlxTrail(dad, null, 4, 24, 0.3, 0.069);
				add(DIEDIE);
		}
		...
		fartStrums = new FlxTypedGroup<StrumNote>();
		...
		if (FlxG.save.data.songPosition) // I dont wanna talk about this code :(
			{
				...
				if (curSong.toLowerCase() == 'restrained')
				{
					songName.setFormat(Paths.font("PAPYRUS.TTF"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		...
		if (curSong.toLowerCase() == 'restrained' || curSong.toLowerCase() == 'bathroom')
		{
			kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 3 ? "Mania" : storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy") + (Main.watermarks ? " - Shred Engine (KE 1.4.2) " + MainMenuState.kadeEngineVer : ""), 16);
			kadeEngineWatermark.setFormat(Paths.font("PAPYRUS.TTF"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		...
		if (curSong.toLowerCase() == 'restrained' || curSong.toLowerCase() == 'bathroom')
		{
			scoreTxt.setFormat(Paths.font("PAPYRUS"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		...
		if (curSong.toLowerCase() == 'restrained' || curSong.toLowerCase() == 'bathroom')
		{
			replayTxt.setFormat(Paths.font("PAPYRUS.TTF"), 42, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		...
		if (isStoryMode)
		{
			switch (curSong.toLowerCase())
			{
				...
				case 'thorns':
					schoolIntro(doof);
				case 'unexpectancies' | 'troubled' | 'restrained' | 'jam' | 'chippin' | 'dejammed':
					schoolIntro(doof);
				default:
					startCountdown();
			}
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				if (unspawnNotes.length > 0)
					...
					oldNote = null;
				var daType = songNotes[keyAmmo[mania] - 1];
				var swagNote:Note = new Note(daStrumTime, daNoteData, oldNote, daType);
		...
		for (i in 0...keyAmmo[mania])
		{
			...
			switch (curStage)
			{
				case 'school' | 'schoolEvil':
					...
					babyArrow.animation.addByPrefix('white', 'arrowSPACE'); //yes this fix playerstrums's position or smth
				...
				default:
					if(FlxG.save.data.noteskin)
						babyArrow.frames = Paths.getSparrowAtlas('NOTE_assets_CIRCLES');
					else
						babyArrow.frames = Paths.getSparrowAtlas('NOTE_assets');
			...
			/*if (SONG.song.toLowerCase() == 'yourmom') //best lines of code
			{[[[[[[[[[[['suck my dick']]]]]]]]]]];
	...
	override public function update(elapsed:Float)
	{
		...
		if (curSong.toLowerCase() == 'restrained')
		{
			...
			{
				spr.x += Math.sin(elapsedtime) * ((spr.ID % 2) == 7 ? 1.25 : -3.25);
				...
				spr.angle += (Math.sin(elapsedtime * 15) + 15) * 15;*/
			});
			...
			/*for(note in notes)
			{
				...
				{
					if (!note.isSustainNote)
						note.angle = strumLineNotes.members[note.noteData].angle;
				}
			}*/
		...
		if (curSong.toLowerCase() == 'bathroom')
		{
			playerStrums.forEach(function(spr:FlxSprite)
				{
					...
					spr.y = ((FlxG.height / 2) - (spr.height / 2)) + (Math.cos(elapsedtime + (spr.ID)) * 269);
					spr.x += Math.sin(elapsedtime) * ((spr.ID % 2) == 0 ? 1 : -1);
					spr.y -= Math.sin(elapsedtime) * 1.3;
				});
			fartStrums.forEach(function(spr:FlxSprite)
				{
					spr.scale.x = Math.abs(Math.sin(elapsedtime - 5) * ((spr.ID % 2) == 0 ? 1 : -1)) / keyAmmo[mania];
		...
		if (SONG.song.toLowerCase() == 'dejammed') 
		{
			playerStrums.forEach(function(spr:FlxSprite)
			{
		...
		{
			if (iconP1.animation.curAnim.name == 'bf-troll')
				iconP1.animation.play(SONG.player1);
			else
		...
		if (FlxG.keys.justPressed.SEVEN)
		{
			...
			{
				case 'restrained':
					PlayState.SONG = Song.loadFromJson("bathroom", "bathroom");
		...
		iconP1.setGraphicSize(Std.int(FlxMath.lerp(150, iconP1.width, 0.75)),Std.int(FlxMath.lerp(150, iconP2.height, 0.5)));
		iconP2.setGraphicSize(Std.int(FlxMath.lerp(150, iconP2.width, 0.75)),Std.int(FlxMath.lerp(150, iconP2.height, 0.5)));
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						if(dad.curCharacter == 'hell-shredboi' || dad.curCharacter == 'CHXPZ')
						{
							FlxG.camera.shake(0.0075, 0.1);
							camHUD.shake(0.0045, 0.1);
						}
						fartStrums.forEach(function(spr:StrumNote)
							{
					...
					if (SONG.song.toLowerCase() == 'bathroom' && (daNote.MyStrum != null))
					{
						if (FlxG.save.data.downscroll)
					...
					else if (SONG.song.toLowerCase() == 'dejammed' && (daNote.MyStrum != null))
					{
						if (FlxG.save.data.downscroll)
	...
	function endSong():Void
	{
		...
		if (offsetTesting)
		{
			FlxG.sound.playMusic(Paths.music('shredMenu1'));
			offsetTesting = false;
		...
		else
		{
			if (isStoryMode)
			{
				...
				{
					if (curSong.toLowerCase() == 'killbot')
					{
						if (health >= 0.1)
						{
	...
	function noteCheck(controlArray:Array<Bool>, note:Note):Void // sorry lol
		{
			...
			if (loadRep)
			{
			...
			else if (controlArray[note.noteData])
				{
					...
					if (mashing <= getKeyPresses(note) && mashViolations < 2) //fuck you anti mash lol
					{
						mashViolations++;
